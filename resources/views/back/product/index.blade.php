            ...
            <div class="box-body">
              <table id="example1" class="table direction table-bordered table-striped">
				...
				<tr>
                  ...
                  <td>{{ $result_page->departments->name }}</td>
                  <td>
          <a href="{{ route('product.active',$result_page->id) }}" class="btn btn-success btn-sm" > {{$result_page->active == '1' ? 'مفعل' : 'غير مفعل'}} </a>
          <a href="{{ route('product.edit',$result_page->id) }}" class="btn btn-info btn-sm" >تعديل</a>
					...
					<form id="delete-form-{{$result_page->id}}" method="post" action="{{ route('product.destroy',$result_page->id) }}">
					@method('DELETE')
					@csrf
					</form>
				  </td>
